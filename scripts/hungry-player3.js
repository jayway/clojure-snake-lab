...
async function main() {
  let data = await getUrl("http://localhost:3000/start/matilda");
  while (true) {
    await wait(100);
    if (data.alive) {
      ...
      data = await getUrl(
        `http://localhost:3000/move/matilda?direction=${direction}`
      );
      if (data.alive === false) console.log("died");
    } else {
      data = await getUrl("http://localhost:3000/start/matilda");
    }
  }
}
